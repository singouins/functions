...
router.post(
  ...
  asyncHandler(async (req: Request, res: Response) => {
    ...
    logger.info(`POST /${item_uuid}/unload`);
    ...
    if (req.item.ammo == 0) {
      const msg = `Item.id:${req.item.id} is already empty (item.ammo == ${req.item.ammo})`
      logger.warn(msg);
      return res.status(200).json({ success: false, msg: msg, payload: req.item });
    }
    ...
    try {
      if (meta.caliber){
        ...
        req.satchel.ammo[meta.caliber as keyof SatchelAmmo] += req.item.ammo;
    ...
    try {
      ...
      logger.error(`Item.id:${req.item.id} Unable to empty item.ammo: ${err}`)
    }
    ...
    const msg = `Item.id:${req.item.id} unloaded`
    logger.verbose(msg);
